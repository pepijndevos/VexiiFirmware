...
void myprint(std::string_view str) {
    soc::uart.write(utils::to_bytes(str));
}
...
// Constants
#define VAR 0x01 // variable
#define SUB 0x02 // substitution (positive variable)
#define NUL 0x03 // null (positive eraser)
#define ERA 0x04 // eraser
#define LAM 0x05 // lambda
#define APP 0x06 // application
#define SUP 0x07 // superposition
#define DUP 0x08 // duplication
#define REF 0x09 // reference
#define OPX 0x0A // operator (reduces x)
#define OPY 0x0B // operator (reduces y, applies the operator)
#define U32 0x0C // unsigned literal
#define I32 0x0D // signed literal
...
void def_new(char* name) {
  if (BOOK.len == BOOK.cap) {
    ...
    BOOK.defs = (Def*)realloc(BOOK.defs, sizeof(Def) * BOOK.cap);
  }

  Def def = {
    .name = name,
    .nodes = (Term*)calloc(RNOD_END, sizeof(Term)),
    .nodes_len = RNOD_END,
    .rbag = (Term*)calloc(RBAG_END, sizeof(Term)),
    .rbag_len = RBAG_END,
  };
...
const char* def_name(Loc def_idx) {
...
void boot(Loc def_idx) {
  if (RNOD_END || RBAG_END) {
    myprint("booting on non-empty state\n");
    //exit(1);
    return;
  }
...
void print_node(Loc loc) {
  ...
  switch (t_tag) {
    ...
    case SUP:
      myprint("{\n");
      print_node(port(1, t_loc));
      print_node(port(2, t_loc));
      myprint("}\n");
      break;
    case U32:
      myprint(itoa(t_loc, buf, 10));
      myprint("\n");
      break;
  }
}